...
public class Paint extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        ComboBox<String> colorComboBox = new ComboBox<>(FXCollections.observableArrayList("Color","BLACK", "BLUE", "GRAY", "GREEN", "MAGENTA", "ORANGE", "RED", "VIOLET", "WHITE", "YELLOW"));
    ...
    private void drawStar(GraphicsContext gc, int row, int column) {
        ...
        double[] yPoints = {y - outerCircleRadius , y - (innerCircleRadius * Math.cos(0.2 * Math.PI)), y - (outerCircleRadius * Math.cos(0.4 * Math.PI)), y - (innerCircleRadius * Math.cos(0.6 * Math.PI)) ,y - (outerCircleRadius * Math.cos(0.8 * Math.PI)), y - (innerCircleRadius * Math.cos( Math.PI)),y - (outerCircleRadius * Math.cos(1.2 * Math.PI)),y - (innerCircleRadius * Math.cos(1.4 * Math.PI)),y - (outerCircleRadius * Math.cos(1.6 * Math.PI)),y - (innerCircleRadius * Math.cos(1.8 * Math.PI))}; 
        double[] xPoints = {x , x - (innerCircleRadius * Math.sin(0.2 * Math.PI)), x - (outerCircleRadius * Math.sin(0.4 * Math.PI)), x - (innerCircleRadius * Math.sin(0.6 * Math.PI)) ,x - (outerCircleRadius * Math.sin(0.8 * Math.PI)),x - (innerCircleRadius * Math.sin( Math.PI)),x - (outerCircleRadius * Math.sin(1.2 * Math.PI)),x - (innerCircleRadius * Math.sin(1.4 * Math.PI)),x - (outerCircleRadius * Math.sin(1.6 * Math.PI)),x - (innerCircleRadius * Math.sin(1.8 * Math.PI))};
    ...
    private void handleMouseDrag(MouseEvent event, GraphicsContext gc) {
       // if ("Draw".equals(slectedShape)){
            int row = (int) (event.getY() / PIXEL_SIZE);
            int column = (int) (event.getX() / PIXEL_SIZE);
           // if (row % (PIXEL_SIZE * brushRadius) == 0 || column % (PIXEL_SIZE * brushRadius) == 0){
            drawPixels(gc, row, column);
          //  }
      //  }
    }
                ...
                if (i >= 0 && i < NUM_PIXELS && j >= 0 && j < NUM_PIXELS) {
                    ...
                    if ("Grid".equals(slectedDesign)){
                        ...
                        if (i < NUM_PIXELS / 5 || j < NUM_PIXELS / 5 || NUM_PIXELS * 0.9 < j ||  NUM_PIXELS * 0.9 < i){
                            if (Math.pow(x, y) % (PIXEL_SIZE * brushRadius) == 0 || Math.pow(y, x) % (PIXEL_SIZE * brushRadius) == 0){
                                gc.fillRect(x, y, PIXEL_SIZE, PIXEL_SIZE);
                            }
                        }  else {
                        if (Math.pow(x / 100 , y) % (PIXEL_SIZE * brushRadius) == 0 || Math.pow(y / 100, x) % (PIXEL_SIZE * brushRadius) == 0){
                            gc.fillRect(x, y, PIXEL_SIZE, PIXEL_SIZE);
                            } 
                        }
                    } else {
